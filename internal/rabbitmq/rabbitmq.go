...
func NewRabbit(ctx context.Context, config *models.Broker, logger *zap.Logger, mongo *db.Mongo, firebase *firebase.Firebase) *RabbitMQ {
	return &RabbitMQ{
		ctx:     ctx,
		config:  config,
		logger:  logger,
...
func (r *RabbitMQ) StartConsumers(
	...
	r.wg.Add(1)
	go r.startConsumer(2, PUSH_NOTIFICATION_EXCHANGE, PUSH_NOTIFICATION_KEY, PUSH_NOTIFICATION_QUEUE)
}
